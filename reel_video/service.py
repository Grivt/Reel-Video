...
from reel_video.services.llm_service import LLMService
from reel_video.services.tts_service import TTSService
from reel_video.services.media import MediaService
from reel_video.services.image_analysis import ImageAnalysisService
from reel_video.services.video_analysis import VideoAnalysisService
from reel_video.services.video import VideoService
from reel_video.services.frame_processor import FrameProcessor
from reel_video.services.persistence import PersistenceService
from reel_video.services.history_manager import HistoryManager
from reel_video.pipelines.standard import StandardPipeline
from reel_video.pipelines.custom import CustomPipeline
from reel_video.pipelines.asset_based import AssetBasedPipeline
...
class ReelVideoCore:
    ...
    Usage:
        ...
        media = await reel_video.media(prompt="a cat")
        
        # Check active capabilities
        print(f"Using LLM: {reel_video.llm.active}")
        print(f"Available TTS: {reel_video.tts.available}")
    
    Architecture (Simplified):
        ReelVideoCore (this class)
    ...
    async def initialize(self):
        ...
        if self._initialized:
            logger.warning("Reel-Video already initialized")
            return
        
        logger.info("🚀 Initializing Reel-Video...")
        ...
        logger.info("✅ Reel-Video initialized successfully\n")
    ...
    def project_name(self) -> str:
        ...
        return self.config.get("project_name", "Reel-Video")
    
    def __repr__(self) -> str:
        """String representation"""
        status = "initialized" if self._initialized else "not initialized"
        pipelines = f"pipelines={list(self.pipelines.keys())}" if self._initialized else ""
        return f"<ReelVideoCore project={self.project_name!r} status={status} {pipelines}>"
